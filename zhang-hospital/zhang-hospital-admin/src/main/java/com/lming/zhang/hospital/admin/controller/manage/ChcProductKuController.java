...
/**
 * ChcProductKucontroller
 * Created by zhanglm on 2018/5/1.
 */
@Controller
@RequestMapping("/manage/chcproductku")
@Api(value = "ChcProductKu控制器", description = "ChcProductKu管理")
public class ChcProductKuController {

    private static final Logger LOGGER = LoggerFactory.getLogger(ChcProductKuController.class);
    ...
    @Autowired
    private ChcProductKuService chcProductKuService;


    @ApiOperation(value = "ChcProductKu首页")
    ...
    public String index(@RequestParam("permissionId") Integer permissionId,
        ...
        return "/manage/chcproductku/index";
    }

    @ApiOperation(value = "ChcProductKu查询")
    @RequiresPermissions("**:read")
    ...
    public Object list(
        ...
        ChcProductKuExample example = new ChcProductKuExample();
        if (!StringUtils.isBlank(sort) && !StringUtils.isBlank(order)) {
            example.setOrderByClause(sort + " " + order);
        }

        List<ChcProductKu> rows = chcProductKuService.selectByExampleForStartPage(example, pageNum, pageSize);
        long total = chcProductKuService.countByExample(example);

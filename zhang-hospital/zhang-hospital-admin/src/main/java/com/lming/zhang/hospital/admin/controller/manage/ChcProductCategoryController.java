...
/**
 * ChcProductCategorycontroller
 * Created by zhanglm on 2018/5/1.
 */
@Controller
@RequestMapping("/manage/chcproductcategory")
@Api(value = "ChcProductCategory控制器", description = "ChcProductCategory管理")
public class ChcProductCategoryController {

    private static final Logger LOGGER = LoggerFactory.getLogger(ChcProductCategoryController.class);
    ...
    @Autowired
    private ChcProductCategoryService chcProductCategoryService;


    @ApiOperation(value = "ChcProductCategory首页")
    ...
    public String index(@RequestParam("permissionId") Integer permissionId,
        ...
        return "/manage/chcproductcategory/index";
    }

    @ApiOperation(value = "ChcProductCategory查询")
    @RequiresPermissions("**:read")
    ...
    public Object list(
        ...
        ChcProductCategoryExample example = new ChcProductCategoryExample();
        if (!StringUtils.isBlank(sort) && !StringUtils.isBlank(order)) {
            example.setOrderByClause(sort + " " + order);
        }

        List<ChcProductCategory> rows = chcProductCategoryService.selectByExampleForStartPage(example, pageNum, pageSize);
        long total = chcProductCategoryService.countByExample(example);

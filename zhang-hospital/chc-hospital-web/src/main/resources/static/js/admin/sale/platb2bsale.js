// 查询下游维护的产品
var queryDownProductUrl ="admin/product/querydn.do";

// 查询下游的购物车
var queryDownCartUrl = 'admin/product/querydowncart.do';
...
function loadCorpCartData(fcorpid){
	
	$("#carDg").datagrid({  
		url:queryDownCartUrl,         //加载的URL
    	isField:"fldId",
    	frozenColumns:[[{field:'ck',checkbox:true}]],  // 冻结列在左侧
    	queryParams:{"tokenid":getTokenid(),"fcorpid":fcorpid},
    	striped:true,
...
function loadingupproduct()
{
	...
	// 初始化加载数据
	$("#upproDg").datagrid({  
		url:queryDownProductUrl,         //加载的URL
    	isField:"fldId",
    	frozenColumns:[[{field:'ck',checkbox:true}]],  // 冻结列在左侧
    	queryParams:{"tokenid":getTokenid(),"fcorpid":fcorpid},
    	striped:true,
...
function createb2bOrder(title,righturl,rightid)
{
    ...
    $("#corpaddressid").combobox({
        url:'public/getdowncorpaddress.json?fcorpid='+fcorpid,
        valueField:'id',
